# run_reichart_supcon_tau005.py
# 依赖: e2e_pipeline.py, e2e_utils.py
# 职责: reichart SupCon tau=0.05 消融（对比 tau=0.07 最优配置）
#
# 背景: supcon_w03_mixoff (tau=0.07) = 0.9641，距 CellChat 0.9655 差 0.0014
#       tau=0.05 更硬的负样本，对 DCM 遗传亚型（TTN/LMNA/RBM20）类内差异更大场景理论更优
#
# 断点: output/supcon_ablation/reichart_supcon_w03_tau005_mixoff.csv
...
DATASET = 'reichart'
TAG = 'supcon_w03_tau005_mixoff'

OVERRIDES = {
    'use_supcon':          True,
    'supcon_weight':       0.3,
    'supcon_tau':          0.05,   # 关键变量：更硬的负样本
    'mc_dropout_T':        15,
    'use_mixup':           False,
    'early_stop_patience': 5,      # 100 epoch 无改善则停，节省时间
}
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  reichart SupCon tau=0.05 实验')
    print(f'  对比: supcon_w03_OFF(tau=0.07)={BASELINE_W03}')
    ...
    print(f'  配置: supcon_weight={OVERRIDES["supcon_weight"]}, tau={OVERRIDES["supcon_tau"]}, '
          f'mixup={OVERRIDES["use_mixup"]}, early_stop_patience={OVERRIDES["early_stop_patience"]}')
