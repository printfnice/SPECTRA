# run_reichart_mc30.py
# 职责: reichart w03_mixoff + MC Dropout T=30（推理采样数翻倍，稳定AUROC估计）
# 断点: output/supcon_ablation/reichart_supcon_w03_mc30.csv
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  reichart w03_mixoff + MC Dropout T=30')
    print(f'  对比: T=15 = {BASELINE_W03}  目标: >{CELLCHAT}')
    print(f'  设备: {device}')
    print(f'{"="*60}')
    ...
    print(f'  vs w03_mc15: {mean_auroc - BASELINE_W03:+.4f}')
    ...
    row = {
        ...
        'mc_dropout_T': 30, 'use_mixup': False,
        'mean_auroc': round(mean_auroc, 4), 'std_auroc': round(std_auroc, 4),
        'delta_vs_w03_mc15': round(mean_auroc - BASELINE_W03, 4),
