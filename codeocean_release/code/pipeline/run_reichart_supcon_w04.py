# run_reichart_supcon_w04.py
# 依赖: e2e_pipeline.py, e2e_utils.py
# 职责: reichart SupCon w=0.4 消融（对比 w=0.3 最优配置 0.9641）
#
# 背景: w=0.1→0.2→0.3 单调递增 0.9463→0.9566→0.9641
#       w=0.4 外推约 +0.005 → 预期 ~0.969，目标超越 CellChat 0.9655
#       tau005 失败（fold1 0.9231 < w03=0.9431），tau 方向放弃
#
# 断点: output/supcon_ablation/reichart_supcon_w04_mixoff.csv
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  reichart SupCon w=0.4 实验')
    print(f'  对比: w=0.3 (tau=0.07) = {BASELINE_W03}')
    ...
    print(f'  配置: supcon_weight={OVERRIDES["supcon_weight"]}, tau={OVERRIDES["supcon_tau"]}, '
          f'mixup={OVERRIDES["use_mixup"]}, early_stop={OVERRIDES["early_stop_patience"]}')
