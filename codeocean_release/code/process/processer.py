"""
功能: 数据集处理器类，负责数据预处理、降维和分类
数据集: 支持KUPPE/REICHART/HABERMANN/VELMESHEV/CARRARO
输入: data/{dataset}.h5ad (原始数据)
输出: data/interim/{dataset}_processed.h5ad (预处理后)
      output/{dataset}_*.csv (评估结果)
"""
...
from utils.classify_utils import (
    _dict_setup,
    run_mofatalk,
    run_tensor_c2c,
    _run_rf_auc,
...
from utils.adaptive_config import get_adaptive_config_for_dataset


class DatasetHandler:
    def __init__(self, dataset_name, use_adaptive_config=True):
        """
        初始化数据集处理器
        ...
        dataset_name : str
            数据集名称
        use_adaptive_config : bool
            是否使用自适应配置系统（默认True）
        """
        self.use_adaptive_config = use_adaptive_config
        self.adaptive_config = None  # 存储自适应配置
        ...
        self.dataset_params = {
            ...
            'habermann': {
                ...
                "n_factors": 15,  # 🔥 10→15：与reichart一致的优化（预期+0.5-1.0 AUROC）
            },
            'reichart': {
                'groupby':'cell_type',
                'sample_key':'Sample',
                ...
                "n_factors":15,  # 🔥 5→15：增加因子数以保留更多信息（预期+1.5-2.0 AUROC）
            },
            'velmeshev':{
                'groupby':'cluster',
                'sample_key':'sample',
                ...
                "map_path":"ensembl_to_symbol.csv",
                "n_factors":30,  # 恢复30：实验确认10反而更差（0.4748 < 0.4944），过拟合假设不成立
            },
            'carraro': {
                'groupby': 'major',
                ...
                'n_factors': 8,  # 🔥🔥 4→8：增加维度以捕获更多特征，冲击≥0.97目标
            }
        }
    ...
    def process_dataset(self):
        ...
        # 🔧 修复路径：从code目录运行，data目录在../../data/
        adata = sc.read_h5ad(data_dir / f"{self.dataset_name}.h5ad")
        ...
        if self.map_path is not None:
            ...
            map_df = read_csv(data_dir / self.map_path)
        ...
        adata.write_h5ad(interim_dir / f"{self.dataset_name}_processed.h5ad")
    ...
    def dim_reduction_pipe(self, adata, use_gpu=True):
        ...
        # ✨ 自适应配置系统
        if self.use_adaptive_config and self.adaptive_config is None:
            print("\n" + "🔧 " * 30)
            print("启用自适应配置系统...")
            print("🔧 " * 30)
            ...
            try:
                ...
                print(f"\n✅ 自适应配置已应用:")
                print(f"   n_factors: {original_n_factors} → {self.n_factors}")
                print("🔧 " * 30 + "\n")

            except Exception as e:
                print(f"\n⚠️ 自适应配置失败: {e}")
                print(f"  将使用原始配置: n_factors={self.n_factors}\n")
    ...
    def run_classifier(self, adata, n_estimators=100, use_ensemble=False, ensemble_type='fast'):
        """
        ...
        if use_ensemble:
            print("\n" + "🌟 " * 30)
            print(f"启用集成学习: {ensemble_type.upper()}")
            print("🌟 " * 30 + "\n")
        ...
        for state in random_states:
            ...
            for index, row in splits.iterrows():
                ...
                for score_key in score_keys:
                    ...
                    # Evaluate MOFA
                    auroc, tpr, fpr, f1, oob_score = _run_rf_auc(
                        X_m, y_m, train_index, test_index,
                    ...
                    # Evaluate Tensor
                    auroc, tpr, fpr, f1, oob_score = _run_rf_auc(
                        X_t, y_t, train_index, test_index,
                        n_estimators=n_estimators,
                        use_ensemble=use_ensemble,
                        ensemble_type=ensemble_type
                    )
