...
class GeneEncoder(nn.Module):
    ...
    可选 ESM-2 先验：用冻结蛋白质语言模型嵌入做残差初始化，
    提供蛋白质功能先验（ReZero 风格，不影响未提供时的 baseline）。
    """
    ...
    def __init__(self, n_genes, gene_emb_dim=64, expr_hidden_dim=128, dropout=0.2,
        ...
        if esm2_features is not None:
            ...
            # 零初始化 adapter，确保初始状态 = 无 ESM-2（保留 baseline）
    ...
    def _init_weights(self):
        ...
        # 重置 ESM-2 adapter 零初始化（_init_weights 会覆盖，需重新置零）
...
class MSHGATLink(nn.Module):
    ...
    def _zero_init_prior_adapters(self):
        """零初始化所有先验 adapter（ESM-2 / KGE），防止 _init_weights Xavier 覆盖。"""
        with torch.no_grad():
