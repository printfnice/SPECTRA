# run_reichart_rdrop_cw.py
# 职责: reichart w03_mixoff + R-Drop(α=0.3) + Class-Weighted CE
#
# 背景: 调研结论 TOP-2 组合
#   R-Drop: 两次 dropout 前向 + KL 一致性（泛化正则，NeurIPS 2021）
#   Class-Weighted CE: Normal(64) vs DCM(107)，少数类权重≈1.67（决策边界校正）
#   两者机制正交，针对 fold1/4 的不同失败根因
# 断点: output/supcon_ablation/reichart_rdrop_cw.csv
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  reichart w03_mixoff + R-Drop(α=0.3) + Class-Weighted CE')
    ...
    print(f'  配置: SupCon_w=0.3, R-Drop_α=0.3, class_weight=ON, '
          f'epochs={base_config["n_epochs"]}')
    ...
    print(f'  有���样本: {n_valid}/{n_samples}')
