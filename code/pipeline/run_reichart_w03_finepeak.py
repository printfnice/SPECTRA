# run_reichart_w03_finepeak.py
# 职责: w03_mixoff + eval_interval=10 + early_stop=0（更密评估，精细找 AUROC 峰值）
# 当前 eval_interval=20，峰值可能落在两次评估之间；改为 10 可多捕获一次
# 断点: output/supcon_ablation/reichart_w03_finepeak.csv
...
OVERRIDES = {
    ...
    'mc_dropout_T':        15,
    'use_mixup':           False,
    'early_stop_patience': 0,      # 禁用 early stop，跑满 300 epoch
    'eval_interval':       10,     # 每 10 epoch 评估一次（默认 20）
    'n_random_states':     3,
}
...
def main():
    ...
    print(f'\n{"="*60}')
    print(f'  reichart w03 + eval_interval=10 + no early stop')
